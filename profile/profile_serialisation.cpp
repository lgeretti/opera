  ...
  public:
    ...
    void profile_bodypresentationpacket() {
        ...
        for (SizeType i=0; i<num_tries(); ++i)
            packets.push_back(Deserialiser<BodyPresentationPacket>(Resources::path("json/examples/presentation/human0.json")).make());
    ...
    void profile_bodystatepacket() {
        ...
        for (SizeType i=0; i<num_samples; ++i)
            packets.push_back(Deserialiser<BodyStatePacket>(Resources::path("json/scenarios/nocollision/h0/"+std::to_string(i+1)+".json")).make());
    ...
    void profile_collisionnotificationpacket() {
        ...
        for (SizeType i=0; i<num_tries(); ++i)
            packets.push_back(Deserialiser<CollisionNotificationPacket>(Resources::path("json/examples/notification/notification0.json")).make());
